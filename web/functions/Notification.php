...
class Notification {
    ...
    function execute() {
        $arr = ['error' => 'Error sending notification! Did you enter a username, password and message to send?']; //This should never happen... but...
        if (empty($this->password) || empty ($this->username) || empty($this->message)) {
            ...
            $pinLayout = new PinLayout(PinLayoutType::GENERIC_PIN, 'Meetup Message', 'Meetup Message', 'From: ' . $this->username, $this->message, PinIcon::GENERIC_EMAIL, PinIcon::GENERIC_EMAIL, PinIcon::GENERIC_EMAIL, PebbleColour::WHITE, PebbleColour::RED);
