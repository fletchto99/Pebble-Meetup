...
(function($, Sortable) {
  ...
  Tabs.prototype._init = function () {
    this.tabs = [].slice.call(this.navbar.querySelectorAll('ul > li'));
    this.sections = [].slice.call(document.querySelectorAll('.content > section'));
    this.current = -1;
    this._show();
    this._initEvents();
    this.onchange = null;
  ...
  Tabs.prototype._show = function (idx) {
    if (this.current >= 0) {
      ...
      this.sections[this.current].className = '';
    }
    this.current = idx != undefined ? idx :  0;
    this.tabs[this.current].className = 'tab-selected';
    this.sections[this.current].className = 'visible';
    if (typeof this.onchange == 'function') {
      var location = this.tabs[this.current].getElementsByClassName('tab')[0].href;
      this.onchange(location.substring(location.lastIndexOf('#') + 1, location.length), idx);
    }
  };
  ...
  $(function() {
    ...
    if (document.getElementById('tabs') !== undefined) {
      Slate.tabs = new Tabs(document.getElementById('tabs'));
    } else {
      Slate.tabs = {};
    }
  });
